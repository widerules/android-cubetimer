...
public class Scrambler {
    ...
    private String buildScramble(int len, Stack<Integer> stack) {
        ...
        // check for stuff like U D U'
        if (stack.size()>2) {
            int last1=(Integer)stack.pop();
            int last2=(Integer)stack.pop();
            int last3=(Integer)stack.pop();
            
            int tmp1 = Math.abs(last1); // will also be a 90 degree rotation
            int tmp2 = (last2>9) ? last2/10 : Math.abs(last2);
            int tmp3 = (last3>9) ? last3/10 : Math.abs(last3);
            
            // check if last move undoes third to last move
            if ((tmp1 == tmp3 && tmp1>tmp2 && tmp1%2==0 && tmp1-tmp2==1) ||
                (tmp1 == tmp3 && tmp2>tmp1 && tmp2%2==0 && tmp2-tmp1==1)) {
                stack.push(last3);
                stack.push(last2);
                return buildScramble(len, stack);
            }
